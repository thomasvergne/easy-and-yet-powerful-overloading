...
Definition update_impls 
  {A: Type} (m: total_map (list A)) (x: string) (v: A) : total_map (list A) :=
    fun y => 
      if String.eqb x y then v :: m y else m y.
...
Lemma includedin_update_impls :
  forall A (m1 m2 : total_map (list A)) x v,
    t_includedin m1 m2 ->
    t_includedin (update_impls m1 x v) (update_impls m2 x v).
Proof.
  intros. unfold t_includedin in *. 
  intros. unfold update_impls in *.
  destruct (String.eqb_spec x x0).
  - subst. apply inj_cons with (x := v). apply H. reflexivity.
